...
public class GetLigneInventaireHandler
    : AbpHandler.With<
          GetLigneInventaireQuery,
          GetLigneInventaireResponse,
          LigneInventaire,
          LigneInventaireDto
      >
{
    ...
    protected override async Task<Result<GetLigneInventaireResponse>> InternalHandle(
    ...
    )
    {
        ...
        if (res == null)
            return NotFound(
                $"Not Found",
                "Aucune ligne d'inventaire n'existe avec l'id {request.Id}"
            );
