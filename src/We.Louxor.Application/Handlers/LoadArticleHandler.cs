...
public class LoadArticleHandler
    : BaseLoadHandler<LoadArticleQuery, LoadArticleResponse, Article, Guid>
{
    ...
    protected override bool CheckForDuplicate(List<Article> records)
    {
        ...
        var query1 = articles.Where(
            c => records.Any(p => c.Societe == p.Societe && c.Code == p.Code)
        );
        if (query1.Count() > 0)
        {
            ...
            {
                Logger.LogWarning(
                    $"{item.Societe}-{item.Code} was duplicates in results".FormatWarning()
                );
            }
