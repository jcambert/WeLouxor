...
{
    protected IRepository<LigneInventaire, Guid> _inv_repository => LazyServiceProvider.LazyGetRequiredService<IRepository<LigneInventaire, Guid>>();
    protected IRepository<Article, Guid> _article_repository => LazyServiceProvider.LazyGetRequiredService<IRepository<Article, Guid>>();
    protected IRepository<OrdreDeFabication, Guid> _of_repository => LazyServiceProvider.LazyGetRequiredService<IRepository<OrdreDeFabication, Guid>>();
    protected IRepository<LigneDeCommande, Guid> _commande_repository => LazyServiceProvider.LazyGetRequiredService<IRepository<LigneDeCommande, Guid>>();
    protected IRepository<Client, Guid> _client_repository => LazyServiceProvider.LazyGetRequiredService<IRepository<Client, Guid>>();
    ...
    public override async Task<AddLigneInventaireResponse> Handle(AddLigneInventaireQuery request, CancellationToken cancellationToken)
    {
        ...
        Func<int, Task<LigneDeCommande>> FindCommandeByNumero = async (numero) =>
        {
            ...
            query2 = from q in query2 where q.Societe == request.Societe && q.NumeroDocument == numero select q;
        ...
        Func<int,Task<LigneDeCommande>> FindCommandeSurcout = async (numero) =>
        {
            ...
            query7 = from q in query7 where q.Societe == request.Societe && q.NumeroDocument > (numero * 1000) && q.NumeroDocument < (numero * 1000 + 999) && q.CodeArticle.StartsWith($"{articleTete.Code}SURCOUT") select q;
            return await AsyncExecuter.FirstOrDefaultAsync(query7, cancellationToken);
        };
        Func<int, int, string, Task<OrdreDeFabication>> FindOfByNumero = async (numero, op, article) =>
        {
            var query3 = await _of_repository.GetQueryableAsync();
            query3 = from q in query3 where q.Societe == request.Societe && q.Numero == numero && q.CodeOperation == op select q;
        ...
        Func<int, string, Task<OrdreDeFabication>> FindOfByNumeroAr = async (ar, article) =>
        {
            ...
            query = from q in query where q.Societe == request.Societe && q.NumeroAR == ar && q.CodeArticle == article select q;
        ...
        if (string.IsNullOrEmpty(request.Article) && (request.OrdreDeFabication == 0 && request.NumeroCommandeClient == 00))
            throw new ArgumentException("La requete sans article doit avoir soit un N° d'Ar soit  un N° d'Of");
        if (request.OrdreDeFabication > 0 && request.CodeOperationFinie == 0)
            throw new AggregateException("Lorsque le N° d'Of est donné, vous devez indiquer le N° de l'operation");
        if (!string.IsNullOrEmpty(request.Article))
        {
            ...
            if (article == null)
                throw new EntityNotFoundException($"L'article {request.Societe}-{request.Article} n'existe pas");
        ...
        if (request.NumeroCommandeClient > 0)
        {
            ...
            if (cde == null)
                throw new EntityNotFoundException($"la commande {request.Societe}-{request.NumeroCommandeClient} avec l'article {article.Code} n'existe pas");
            if (article == null)
                //Recherche de l'article inventorie
        ...
        if (request.OrdreDeFabication > 0)
        {
            ...
            of = await FindOfByNumero(request.OrdreDeFabication, request.CodeOperationFinie, request.Article);
            if (of == null && !string.IsNullOrEmpty(request.Article))
                throw new EntityNotFoundException($"L'of {request.Societe}-{request.OrdreDeFabication}-{request.CodeOperationFinie} avec l'article {article.Code} n'existe pas");
            else if (of == null)
                throw new EntityNotFoundException($"L'of {request.Societe}-{request.OrdreDeFabication}-{request.CodeOperationFinie}  n'existe pas");
            if (cde == null)
            {
                ...
                request.NumeroCommandeClient = cde != null ? cde.NumeroDocument : request.NumeroCommandeClient;
            ...
            if (article == null)
            {
                ...
                if (article == null)
                    throw new EntityNotFoundException($"L'article {of.CodeArticle} provenant de l'Of {of.Numero} n'existe pas");
            }
        ...
        if (cde != null)
        {
           ...
           /* var query5 = await _article_repository.GetQueryableAsync();
            ...
            if (articleTete == null)
                throw new EntityNotFoundException($"L'article de tete {request.Societe}-{cde.CodeArticle} n'existe pas");
            if (client == null)
            {
        ...
        LigneInventaire inventaire = new()
        {
            Page = request.Page,
