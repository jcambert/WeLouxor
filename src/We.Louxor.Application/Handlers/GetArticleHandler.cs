...
public class GetArticleHandler : BaseHandler<GetArticleQuery, GetArticleResponse>
{
    ...
    public override async Task<GetArticleResponse> Handle(GetArticleQuery request, CancellationToken cancellationToken)
    {
        var query = await repository.GetQueryableAsync();
        query = from q in query where q.Societe == request.Societe && q.Code == request.Code select q;
