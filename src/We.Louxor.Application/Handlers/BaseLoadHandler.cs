    ...
    where TResponse : new()
{
    ...
    )
    {
        ...
        {
            request.Filename = request.Filename.EnsureEndsWith(".dbf");
            ...
            if (!fi.Exists)
                throw new ArgumentException(
                    $"{request.Filename} does not exist in wwww/database/folder "
                );
            ...
            using (var reader = new BinaryReader(stream))
            {
                ...
                Dbf.AllRecordsLoaded.Subscribe(
                    b =>
                    {
                        Logger.LogDebug(
                            $"{Path.GetFileNameWithoutExtension(request.Filename)} All records Loaded successfully".FormatColor()
                        );
                    }
                );
                Dbf.RecordsLoaded.Subscribe(
                    records =>
                    {
                        Logger.LogDebug($"{records.Count} where loaded".FormatColor());
                        List<TEntity> lignes = ObjectMapper.Map<List<RecordData>, List<TEntity>>(
                            records
                        );
                        if (request.TestForDuplicate)
                        {
                            ...
                            {
                                Logger.LogWarning(
                                    "It seem there are duplicates values!!".FormatWarning()
                                );
                            }
                        }
                        else
                        {
                            ...
                            Task.Run(
                                    async () =>
                                    {
                                        ...
                                        Logger.LogDebug(
                                            $"{lignes.Count} lines were inserted in database. Total:{counter}/{Dbf.TotalRecord}".FormatColor()
                                        );
                                    }
                                )
                                .GetAwaiter()
    ...
    protected virtual Func<TEntity, bool> GetPredicateFilter() =>
        (TEntity entity) =>
        {
            if (Request != null)
